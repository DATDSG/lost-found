...
class User(Base):
    ...
    status = Column(String, default="active")  # Added: matches database column
...
class Report(Base):
    ...
    embedding = Column(Vector(384))  # E5-small dimension for semantic search
    image_hash = Column(String(32), index=True)  # Perceptual hash from Vision service
...
class Match(Base):
    ...
    status = Column(SQLEnum(MatchStatus), default=MatchStatus.CANDIDATE, index=True)
...
class Conversation(Base):
    ...
    participant_one_id = Column(String, ForeignKey("users.id"), nullable=False)
    participant_two_id = Column(String, ForeignKey("users.id"), nullable=False)
...
class AuditLog(Base):
    ...
    id = Column(String, primary_key=True)
    actor_id = Column(String, ForeignKey("users.id"))  # Fixed: matches actual column name
