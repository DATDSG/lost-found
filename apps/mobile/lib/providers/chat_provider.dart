...
class ChatProvider extends StateNotifier<ChatState> {
  ...
  void _handleTypingIndicator(Map<String, dynamic> message) {
    ...
    if (conversationId != null && userId != null) {
      ...
      if (isTyping == true) {
        ...
        updatedTypingUsers = currentTypingUsers.where((id) => id != userId).toList();
      }
  ...
  Future<ChatConversation?> createConversation(String itemId, String recipientId) async {
    try {
      final conversationId = await _apiService.createConversation(itemId, recipientId);
      if (conversationId != null) {
