...
class _ReportFoundScreenState extends State<ReportFoundScreen> {
  ...
  Future<void> _submitReport() async {
    ...
    final success = await reportsProvider.createReport(
      ...
      category: _category,
      city: _city,
      occurredAt: _foundAt ?? DateTime.now(),
    ...
    if (success && mounted) {
      Navigator.of(context).pushReplacementNamed(
        AppRoutes.success,
        arguments: {'type': 'found', 'title': _title},
      );
    } else if (mounted) {
  ...
  Widget _buildBasicInfoStep() {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Text('What did you find?', style: DT.t.headline2),
          SizedBox(height: DT.s.sm),
          Text(
            'Provide a clear title for the item you found',
          ...
          FormFieldWrapper(
            child: InkWell(
              ...
              child: InputDecorator(
                ...
                child: Text(
                  _foundAt != null
                      ? '${_foundAt!.day}/${_foundAt!.month}/${_foundAt!.year}'
                      : 'Select date',
                  style: TextStyle(
                    color: _foundAt != null ? DT.c.text : DT.c.textMuted,
                  ),
                ),
              ),
            ),
          ...
          FormFieldWrapper(
            child: SwitchListTile(
              title: const Text('Offer Reward'),
              subtitle: const Text('Let others know you\'re offering a reward'),
  ...
  Widget _buildDescriptionStep() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Describe the item', style: DT.t.headline2),
          SizedBox(height: DT.s.sm),
          Text(
            'Add details that will help the owner identify their item',
            style: DT.t.bodyMuted,
          ),
          SizedBox(height: DT.s.xl),
  ...
  Widget _buildLocationStep() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Where did you find it?', style: DT.t.headline2),
          SizedBox(height: DT.s.sm),
          Text(
            'Help the owner know where to look for their item',
            style: DT.t.bodyMuted,
          ),
          SizedBox(height: DT.s.xl),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: DT.s.sm),
          Text(
            'Photos help the owner identify their item (optional)',
            style: DT.t.bodyMuted,
          ),
          SizedBox(height: DT.s.xl),
