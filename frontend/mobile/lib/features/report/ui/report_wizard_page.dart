...
class _ReportWizardPageState extends State<ReportWizardPage> {
  ...
  String get _title => widget.type == ReportType.found
      ? 'Report Found Item'
      : 'Report Lost Item';
  ...
  Future<void> _submit() async {
    ...
    final title = _brandCtrl.text.trim().isEmpty
        ? 'Item'
        : _brandCtrl.text.trim();
    final buffer = StringBuffer();
    if (_category.value != null) buffer.writeln('Category: ${_category.value}');
    if (_colorCtrl.text.trim().isNotEmpty) {
      buffer.writeln('Color: ${_colorCtrl.text.trim()}');
    }
    if (_marksCtrl.text.trim().isNotEmpty) {
      buffer.writeln('Marks: ${_marksCtrl.text.trim()}');
    }
    if (_rewardCtrl.text.trim().isNotEmpty && widget.type == ReportType.lost) {
      buffer.writeln('Reward: ${_rewardCtrl.text.trim()}');
    }
    if (_when != null) {
      buffer.writeln('When: ${_when!.toLocal()}');
    }

    setState(() => _submitting = true);
    try {
      final created = await ItemApi.I.createItem(
        title: title,
        description: buffer.toString().trim().isEmpty
            ? null
            : buffer.toString().trim(),
        status: widget.type == ReportType.found ? 'found' : 'lost',
        lat: _pin?.latitude,
        lng: _pin?.longitude,
      );
      if (!mounted) return;
      Navigator.pop<ItemDto>(context, created);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
            ...
            for (final c in [
              ...
              _ColorDot(
                ...
                onTap: () => colorCtrl.text =
                    '#${c.toARGB32().toRadixString(16).substring(2).padLeft(6, '0')}',
              ),
          ],
        ),
        ...
        _input(
          marksCtrl,
          hint: 'Any distinguishing features? (e.g., scratches, stickers)',
          minLines: 2,
          maxLines: 4,
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const _Rule(text: 'Avoid including personal information.', bad: true),
        SizedBox(height: DT.s.md),
        GridView.count(
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Public sees fuzzy location until chat is approved',
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(
            bad ? Icons.close_rounded : Icons.check_circle_rounded,
